...
- `E0-006` SegmentIdentityHistoryは2026-09-23に既存 `segment_identity.py` をWEB-009/E0-005へ再照合した。stable segment_id、classification role、rename/merge/split/recast、as_reported_at/filing accession境界を確認し、cross-instrument edge禁止とRECAST同一identity制約を追加した。現在は **実装完了 / local acceptance pending**。受入記録は `REPORT_DEV_E0_006_SEGMENT_IDENTITY_HISTORY_ACCEPTANCE_2026-09-23.md` を参照する。
...
| WEB-018 | O0-004 | C | 引渡し済み | WEB-001/015/017引渡し済み | [`REPORT_OFFICIAL_INSTRUMENT_THEME_LINKAGE_WEB_018_2026-09-04.md`](REPORT_OFFICIAL_INSTRUMENT_THEME_LINKAGE_WEB_018_2026-09-04.md); Evidence確認 `2026-09-04T09:09:35Z` | O0-004 instrument/theme Relationship、I0-005、I0-007、WEB-019 | `DIRECT_INSTRUMENT`はidentity + action/effect anchorを必須化し、`THEME_EXPOSURE`はofficial semiconductor scopeだけへ付与。`MENTION_ONLY`/`UNRESOLVED`/`NO_LINK` negative fixtureを実装し、precision/recallはlocal実測 | 2026-09-04 | web-2026-09-04-WEB-018 |
| WEB-019 | O0-005 | D | 保留（依存） | WEB-016〜018引渡し済み、残りlocal adapter/contract形状 | WEB-016でupdate/delete/重複/時刻、WEB-017でsemantic Fact、WEB-018でdirect/theme/mention linkage fixture候補を準備済み | Official Signal品質test | local adapter/Relationship形状確定後にsemantic/linkage fixtureを統合し品質setを完成 | 2026-09-04 | — |
...
16. `E0-006`: SegmentIdentityHistory — edge invariant補強済み、local acceptance待ち。
17. PASS後に `E0-007` earnings Canary quality reportへ進む。
...
| `I0-007` | **Accepted (2026-09-22)** | UTC/max_pages追加差分後、CodexDesktop側4契約テスト55件PASS・cleanを確認 |
...
- 主経路の次セッションは `E0-006` local acceptance確認から開始する。
