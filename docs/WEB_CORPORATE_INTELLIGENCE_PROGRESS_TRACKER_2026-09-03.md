...
- S0-002〜006が揃ったため、Corporate Intelligence主経路の次作業は `S0-007 Filing-detection acceptance test` である。
...
10. **次に `S0-007` Filing-detection acceptance testを正式完了する。**
...
- 主経路の次セッションは `S0-007` Filing-detection acceptance testから開始する。
