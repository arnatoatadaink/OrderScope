...
- `E0-003` company-IR fallbackは2026-09-23に既存 `ir_fallback.py` をWEB-008/E0-001/E0-002/I0契約へ再照合した。canonical URL/hash、duplicate discovery、SEC/IR Evidence保持に加えて、retrieved_atとCOMPLETE/PARTIAL/ERROR acquisition境界を追加した。現在は **実装完了 / local acceptance pending**。受入記録は `REPORT_DEV_E0_003_COMPANY_IR_FALLBACK_ACCEPTANCE_2026-09-23.md` を参照する。
...
13. `E0-003`: company-IR fallback — acquisition status/retrieved_at境界追加済み、local acceptance待ち。
14. PASS後に `E0-004` basic earnings Factsへ進む。
...
- 主経路の次セッションは `E0-003` local acceptance確認から開始する。
