...
- `E0-002` SEC earnings detectionは2026-09-23に既存実装をS0-007/E0-001/WBSへ再照合した。10-Q/10-Kは直接candidate、8-K/8-K-AはItem 2.02またはearnings意味を明示した99.x attachmentのみcandidateとする境界を確認し、8-K/A fixtureを追加した。現在は **実装完了 / local acceptance pending**。受入記録は `REPORT_DEV_E0_002_SEC_EARNINGS_DETECTION_ACCEPTANCE_2026-09-23.md` を参照する。
...
12. `E0-002`: SEC earnings detection — 8-K/A fixture追加済み、local acceptance待ち。
13. PASS後に `E0-003` company-IR fallbackへ進む。
...
- 主経路の次セッションは `E0-002` local acceptance確認から開始する。
