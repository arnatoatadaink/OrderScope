...
- `I0-002`〜`I0-007` の共通contract chainは完了した。Corporate Intelligence主経路の次作業は `S0-002 Implement CIK/submissions adapter` である。
...
6. **次に `S0-002` CIK/submissions adapterを実装する。**
...
- 主経路の次セッションは `S0-002` CIK/submissions adapterから開始する。
