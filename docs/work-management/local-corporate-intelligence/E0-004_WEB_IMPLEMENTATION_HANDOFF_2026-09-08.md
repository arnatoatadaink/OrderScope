...
Exact duplicate observations from the same normalized source input collapse idempotently.
...
```bash
uv run pytest -q analysis/tests/earnings/test_basic_facts.py
uv run pytest -q
git diff --check
```
