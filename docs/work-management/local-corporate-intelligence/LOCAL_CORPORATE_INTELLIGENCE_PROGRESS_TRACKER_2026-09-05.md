...
| L1-003 | INCONCLUSIVE — four historical-recovery chunks accepted remotely | Version-8 and version-9 one-shot windows completed the September 3 Regular session and advanced the clean NVDA checkpoint from version 8 to 10 / `2026-09-03T20:00:00.000Z`. Further continuation, normal-scheduler handoff, stability observation, and Phase B remain separately gated. |
...
6. Treat the first, version-7, version-8, and version-9 NVDA historical-recovery change windows as remotely accepted. The current clean checkpoint is version 10 through `2026-09-03T20:00:00.000Z`. Any continuation campaign requires a fresh moving-retention-horizon preflight and reviewed campaign shape; only after recovery reaches the re-frozen handoff boundary, the unchanged normal scheduler takes ownership, and at least two clean scheduler opportunities are observed may a reviewed market-session Phase B window create a fresh pause-created gap. Keep purge/`PURGED`, scheduler-evidence activation, remote backup/restore, and other Worker/Cron mutations separately gated.
...
| L1-003 Phase B start-readiness WBS/CP | PB-01/PB-02 planning and PB-03 local campaign driver accepted; one Regular session is frozen as four independent 100/100/100/90-bar calls, with checkpoint/accepted-record re-read, fail-stop, replay closure, resumable frozen boundaries, per-invocation budget checks and no cross-session continuation; focused TypeScript 7, full TypeScript 199, full Python 696, typecheck, generated types, live-canary dry-run and diff check passed; PB-04 requires a fresh moving-horizon preflight and separate authorization |
...
- L1-003 / SMOKE-007 PB-01/PB-02 planning and PB-03 one-session operator campaign are locally accepted. Repeat the moving-horizon preflight, freeze the exact next session identities, and separately authorize PB-04; no recovery execution is automatic. After recovery, hand off to the unchanged normal scheduler and observe at least two clean opportunities. Only after those gates may a separately authorized market-session Phase B window create and catch up a fresh pause-created gap.
