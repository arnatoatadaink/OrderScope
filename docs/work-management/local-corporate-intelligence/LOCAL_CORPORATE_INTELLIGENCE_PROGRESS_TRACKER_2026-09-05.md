...
| L1-003 | INCONCLUSIVE — two historical-recovery chunks accepted remotely | The bounded continuation window inserted the next 100 canonical NVDA bars and advanced the clean checkpoint from version 7 to 8 / `2026-09-03T16:50:00.000Z`. Further continuation, normal-scheduler handoff, and Phase B remain separately gated. |
...
6. Treat the first and version-7 NVDA historical-recovery change windows as remotely accepted. The current clean checkpoint is version 8 through `2026-09-03T16:50:00.000Z`. Any next chunk requires a new read-only preflight and deterministic version-8 candidate review; only after recovery reaches the reviewed handoff boundary, normal-scheduler handoff, and current clean checkpoint evidence may a reviewed market-session Phase B window create a fresh pause-created gap. Keep purge/`PURGED`, scheduler-evidence activation, remote backup/restore, and other Worker/Cron mutations separately gated.
...
- L1-003 / SMOKE-007 next deterministic version-8 continuation review/window, eventual normal-scheduler handoff, and current-session stability evidence; only then a separately authorized market-session Phase B window for fresh pause-created catch-up. Phase A and the first two 100-bar remote chunks are accepted.
