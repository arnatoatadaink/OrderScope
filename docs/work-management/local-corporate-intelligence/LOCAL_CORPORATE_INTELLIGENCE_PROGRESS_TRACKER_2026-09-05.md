...
| L1-003 | INCONCLUSIVE — first historical-recovery chunk accepted; continuation locally accepted | The first remote NVDA chunk inserted 100 canonical bars and advanced the clean checkpoint from version 6 to 7. A per-call version/boundary/job-bound continuation endpoint is locally accepted; its deploy/execution and Phase B remain separately gated. |
...
6. Treat the first NVDA historical-recovery change window as remotely accepted and the per-call continuation mechanism as locally accepted. The next `L1-003/SMOKE-007` action is a separately authorized version-7 read-only preflight and one-chunk continuation window for job `historical-market-recovery:aac843c098bae3a3`; only after recovery reaches the reviewed handoff boundary, normal-scheduler handoff, and current clean checkpoint evidence may a reviewed market-session Phase B window create a fresh pause-created gap. Keep purge/`PURGED`, scheduler-evidence activation, remote backup/restore, and other Worker/Cron mutations separately gated.
...
- L1-003 / SMOKE-007 separately authorized version-7 continuation preflight/deploy/one-chunk window, eventual normal-scheduler handoff, and current-session stability evidence; only then a separately authorized market-session Phase B window for fresh pause-created catch-up. The continuation implementation is local-only; Phase A and the first 100-bar remote chunk are accepted.
