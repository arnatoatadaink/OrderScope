...
6. Treat the earlier one-shot windows and the first PB-04 September 4 session campaign as remotely accepted. The PB-05 read-only assessment at `2026-09-18T01:44:18Z` found the current clean checkpoint still at version 14 through `2026-09-04T20:00:00.000Z`; zero bars exist between that close and the frozen September 16 boundary, so PB-05 is blocked by seven sessions / 2,730 bars of unfinished PB-04 work. Any next-session campaign requires a fresh moving-retention-horizon preflight and a separately authorized one-session window; only after recovery reaches the re-frozen handoff boundary, the unchanged normal scheduler takes ownership, and at least two clean scheduler opportunities are observed may a reviewed market-session Phase B window create a fresh pause-created gap. Keep purge/`PURGED`, scheduler-evidence activation, remote backup/restore, and other Worker/Cron mutations separately gated.
