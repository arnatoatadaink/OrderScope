...
6. Treat the earlier one-shot windows and the first PB-04 September 4 session campaign as remotely accepted. The current clean checkpoint is version 14 through `2026-09-04T20:00:00.000Z`. Any next-session campaign requires a fresh moving-retention-horizon preflight and a separately authorized one-session window; only after recovery reaches the re-frozen handoff boundary, the unchanged normal scheduler takes ownership, and at least two clean scheduler opportunities are observed may a reviewed market-session Phase B window create a fresh pause-created gap. Keep purge/`PURGED`, scheduler-evidence activation, remote backup/restore, and other Worker/Cron mutations separately gated.
