...
2026-09-22時点で、Windows側source + WSL runtimeへの移行は **完了**。MIG-09E正式受入、MIG-09F Git最終確認、CodexDesktop側検収合格を通過した。通常開発へ復帰し、`DEV-I0-002`〜`DEV-I0-007` は正式受入まで完了した。現在の本流は `E0-004 basic earnings Facts` のlocal acceptanceである。
...
| DEV-E0-004 | basic earnings Facts | **実装完了 / local acceptance pending** | revenue/net income/EPSのFact/Evidence化、period/unit/source、非推測、idempotencyを再照合。`REPORT_DEV_E0_004_BASIC_EARNINGS_FACTS_ACCEPTANCE_2026-09-23.md`参照 |
