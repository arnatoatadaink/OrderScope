...
PACKET="${PB04_PACKET:-var/l1-003/local-market-recovery/nvda/PB04_NVDA_2026-09-14_REMOTE_PACKET.json}"
EVIDENCE="${PB04_EVIDENCE:-var/l1-003/local-market-recovery/nvda/NVDA_1Min_REGULAR_2026-09-14.json}"
...
if (packet.schemaVersion !== "l1-003-pb04-remote-execution-packet-v1"
  ...
  || packet.frozenRemoteCheckpoint?.version !== 30
  || packet.frozenRemoteCheckpoint?.completeThrough !== "2026-09-11T20:00:00.000Z"
  || packet.source?.marketDate !== "2026-09-14"
  || packet.source?.contentSha256 !== evidence.contentSha256
  || evidence.contentSha256 !== "d8e608efbedbef1c0657de01709ce7cfda594d3108355724c26bf56cee68fa55"
  || packet.recoveryId !== "L1-003-NVDA-20260914-LOCAL"
  || packet.chunks?.length !== 4) {
  throw new Error("Sep14 packet/evidence frozen identity mismatch");
}
...
echo
echo "== execute frozen Sep14 four-chunk campaign =="
...
echo
echo "== final Sep14 checkpoint =="
final_sql="
SELECT CASE WHEN COUNT(*) = 1 THEN 1 ELSE 0 END AS final_ok
FROM coverage_checkpoint
WHERE coverage_key = '${COVERAGE_KEY}'
  AND version = 34
  AND complete_through = '2026-09-14T20:00:00.000Z'
  AND state = 'COMPLETE'
  AND missing_ranges_json = '[]'
  AND blocker_json IS NULL;
"
...
echo "PB-04 Sep14 campaign: ACCEPTED REMOTELY at v34 / 2026-09-14T20:00:00.000Z"
